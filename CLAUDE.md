# Umbraco.Mcp.HostedAuth (v17 line)
...
This is the CMS 17 line (`v17/main` / `v17/dev`). The CMS 18 line lives on
`main` / `dev` in the same repo — see its own copy of this file for anything
17-vs-18 specific.
...
- `v17/main` / `v17/dev` (this line) — CMS 17 (`Umbraco.Cms.Api.Management [17.3.0, 18.0.0)`)
...
NuGet listing, not two — that's why `umbraco-marketplace.json` only exists on
`main` (the repo's default branch, not this one): the Marketplace only needs
it once.
...
- Day-to-day work branches off `v17/dev`, PRs back into it, **squash merge**.
- A release branches off `v17/dev` into `release/<version>`. **The only file
  that needs a version bump is `Directory.Build.props`'s `<Version>`** —
  nothing else hardcodes it. PR into `v17/main` with a **merge commit, not
  squash** — the tag automation and the main→dev sync both key off that
  commit.
- Landing on `v17/main` triggers, in order:
  1. `build/azure-pipelines.yml`'s `Publish` stage → the `umbracoprereleases`
     MyGet feed. Fires on every push to `v17/main`, which under gitflow
     means every merged release, not every commit.
...
- **`build/azure-pipelines.yml`'s path is not yet live.** The Azure DevOps
  ...
  portal equivalent) until the move has actually landed on **both**
  `v17/main` and `main` via a real release — flip it too early and the next
...
- `umbraco-marketplace.json` lives on `main` only — don't add a copy here.
